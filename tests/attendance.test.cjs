...
test('service worker cache/assets and injection agree; injection is idempotent',()=>{
  const events={},ctx={self:{addEventListener:(name,fn)=>events[name]=fn}};ctx.importScripts=()=>{ctx.self.COMBAT_APP={cache:'combat-equipment-v70'}};vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8')+'\nthis.test={CACHE,ASSETS};',ctx);
  const a=ctx.test,html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.equal(a.CACHE,'combat-equipment-v70');
  ...
  assert.ok(a.ASSETS.includes('./app-lifecycle.js?v=8'));assert.ok(html.includes('./app-lifecycle.js?v=8'));
  assert.ok(a.ASSETS.includes('./assignment-transfer.js?v=1'));assert.ok(html.includes('./assignment-transfer.js?v=1'));
  assert.ok(a.ASSETS.includes('./cloud-config.js?v=2'));assert.ok(html.includes('./cloud-config.js?v=2'));
  assert.ok(a.ASSETS.includes('./cloud-bundle.js?v=4'));assert.ok(html.includes('./cloud-bundle.js?v=4'));
