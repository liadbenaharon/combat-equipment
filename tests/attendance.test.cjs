...
test('service worker cache/assets and injection agree; injection is idempotent',()=>{
  const events={},ctx={self:{addEventListener:(name,fn)=>events[name]=fn}};ctx.importScripts=()=>{ctx.self.COMBAT_APP={cache:'combat-equipment-v63'}};vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8')+'\nthis.test={CACHE,ASSETS};',ctx);
  const a=ctx.test,html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.equal(a.CACHE,'combat-equipment-v63');
  ...
  assert.ok(a.ASSETS.includes('./returns.js?v=6'));assert.ok(html.includes('./returns.js?v=6'));
...
test('service worker installs new cache and serves enhanced HTML/assets offline',async()=>{
  ...
  events.activate({waitUntil:p=>pending=p});await pending;assert.deepEqual(deleted,['combat-equipment-v21','combat-equipment-v62']);
