...
function harness(seed={}){
  ...
  vm.runInContext(source.slice(0,source.indexOf('  let observerBusy='))+`\nwindow.test={normalizePhone,makeContact,parseContacts,validateContacts,readContacts,writeContacts,contactPhone,parseList,statusOf,similarName,analyzeNoShowsWithGear,noShowsWithGear,wasAsked,setAsked,transferEquipment,openWhatsApp,copyMessage,checkAssignedNoShows,deleteSelectedContacts,selectedContacts,updateContactSelection,clearContactSelection,renderContacts,select:setSelectedWorkout};})();`,ctx);
...
test('attendance matches a short equipment name to a unique full attendance name',()=>{
  ...
  assert.equal(a.statusOf('תומר'),'ambiguous');
  assert.equal(a.noShowsWithGear().length,0);
});
test('attendance safely matches surnames and one-letter Hebrew spelling variants',()=>{
  const {api:a,ctx,data}=harness();
  data.set(AK,JSON.stringify({current:{attending:[],absent:['שי גרובמן','רועי']}}));
  ctx.state.equipment=[{id:'water',name:'מים',assignments:[{name:'גרובמן',qty:1},{name:'רואי',qty:2}]}];
  assert.equal(a.similarName('גרובמן','שי גרובמן'),true);
...
test('transferring to an existing summary person merges their equipment assignment',()=>{
  ...
  assert.match(elements.noShowWarnings.innerHTML,/הציוד הועבר ל־<b>איתי<\/b>/);
});
test('transferring equipment in a historical workout persists the new holder',()=>{
  const {api:a,data,elements}=harness();
  data.set('combatEquipmentHistoryV1',JSON.stringify([{id:'past',date:'past',people:[{name:'גרובמן',items:[{name:'אלונקה',qty:1}]}],equipment:[{id:'stretcher',name:'אלונקה',assignments:[{name:'גרובמן',qty:1,units:[0]}]}]}]));
  data.set(AK,JSON.stringify({'history:past':{attending:[],absent:['שי גרובמן']}}));elements.attendanceResults={innerHTML:''};elements.noShowWarnings={innerHTML:''};a.select('history:past');
  assert.equal(a.transferEquipment('גרובמן','תומר'),true);
  const saved=JSON.parse(data.get('combatEquipmentHistoryV1'))[0];
  assert.equal(saved.equipment[0].assignments[0].name,'תומר');
  assert.equal(saved.people[0].name,'תומר');
  assert.match(elements.noShowWarnings.innerHTML,/הציוד הועבר ל־<b>תומר<\/b>/);
});
test('selected historical workout and transfer notice survive a full reload',()=>{
  ...
  const {api:a,elements}=harness({combatEquipmentHistoryV1:history,[AK]:attendance,combatEquipmentAttendanceWorkoutV1:'history:past'});elements.noShowWarnings={innerHTML:''};a.checkAssignedNoShows();
...
test('two similar equipment names are reported as ambiguous instead of guessed',()=>{
  ...
  ctx.state.equipment=[{id:'water',name:'מים',assignments:[{name:'רועי כהן',qty:1},{name:'רואי לוי',qty:1}]}];
  const result=a.analyzeNoShowsWithGear();
  assert.equal(result.people.length,0);assert.equal(result.ambiguities.length,1);
  assert.equal(result.ambiguities[0].names.join('|'),'רועי כהן|רואי לוי');
  elements.noShowWarnings={innerHTML:''};a.checkAssignedNoShows();
  assert.match(elements.noShowWarnings.innerHTML,/נמצאו 2 התאמות אפשריות/);
  assert.match(elements.noShowWarnings.innerHTML,/צריך לבדוק מולם/);
});
...
test('service worker cache/assets and injection agree; injection is idempotent',()=>{
  const events={},ctx={self:{addEventListener:(name,fn)=>events[name]=fn}};ctx.importScripts=()=>{ctx.self.COMBAT_APP={cache:'combat-equipment-v56'}};vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8')+'\nthis.test={CACHE,ASSETS};',ctx);
  const a=ctx.test,html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.equal(a.CACHE,'combat-equipment-v56');
  for(const asset of a.ASSETS)assert.ok(fs.existsSync(path.join(root,asset.split('?')[0])),asset);
  assert.ok(a.ASSETS.includes('./attendance.js?v=16'));assert.ok(html.includes('./attendance.js?v=16'));
...
test('service worker installs new cache and serves enhanced HTML/assets offline',async()=>{
  ...
  const ctx={Response,URL,fetch:async()=>{throw Error('offline')},self:{location:{origin:'https://example.test'},addEventListener:(name,fn)=>events[name]=fn,skipWaiting:async()=>{},clients:{claim:async()=>{} }},caches:{open:async()=>({addAll:async assets=>{installed=assets},put:async(k,v)=>cache.set(k,v)}),keys:async()=>['combat-equipment-v21','combat-equipment-v55'],delete:async k=>deleted.push(k),match:async k=>cache.get(k)}};ctx.importScripts=()=>{ctx.self.COMBAT_APP={cache:'combat-equipment-v56'}};
  vm.createContext(ctx);vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8'),ctx);
  let pending;events.install({waitUntil:p=>pending=p});await pending;assert.ok(installed.includes('./attendance.js?v=16'));
  events.activate({waitUntil:p=>pending=p});await pending;assert.deepEqual(deleted,['combat-equipment-v21','combat-equipment-v55']);
