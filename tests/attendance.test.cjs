...
test('historical transfer moves only units that were not returned',()=>{
  ...
  const {api:a,ctx,data,elements}=harness({combatEquipmentHistoryV1:JSON.stringify(history),combatEquipmentReturnsV1:JSON.stringify(returns),[AK]:JSON.stringify({'history:past':{attending:[],absent:['מאליק']}})});
  ctx.state.equipment=[{id:'stretcher',name:'אלונקה',qty:3,checked:[false,true,true],assignments:[{name:'מאליק',qty:2,units:[1,2],carriedFromWorkoutId:'past'}]}];
  ...
  assert.equal(ctx.state.equipment[0].assignments[0].name,'רימון');
...
test('latest historical workout is always the default and its transfer notice survives reload',()=>{
  ...
  const {api:a,elements}=harness({combatEquipmentHistoryV1:history,[AK]:attendance,combatEquipmentAttendanceWorkoutV1:'history:older'});elements.noShowWarnings={innerHTML:''};a.checkAssignedNoShows();
  assert.equal(a.selected(),'history:past');
  assert.match(a.options()[1].label,/^האימון האחרון · /);
...
test('service worker cache/assets and injection agree; injection is idempotent',()=>{
  const events={},ctx={self:{addEventListener:(name,fn)=>events[name]=fn}};ctx.importScripts=()=>{ctx.self.COMBAT_APP={cache:'combat-equipment-v73'}};vm.createContext(ctx);
  vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8')+'\nthis.test={CACHE,ASSETS};',ctx);
  const a=ctx.test,html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.equal(a.CACHE,'combat-equipment-v73');
  ...
  assert.ok(a.ASSETS.includes('./app-lifecycle.js?v=10'));assert.ok(html.includes('./app-lifecycle.js?v=10'));
  assert.ok(a.ASSETS.includes('./assignment-transfer.js?v=3'));assert.ok(html.includes('./assignment-transfer.js?v=3'));
  assert.ok(a.ASSETS.includes('./cloud-config.js?v=2'));assert.ok(html.includes('./cloud-config.js?v=2'));
  assert.ok(a.ASSETS.includes('./cloud-bundle.js?v=5'));assert.ok(html.includes('./cloud-bundle.js?v=5'));
