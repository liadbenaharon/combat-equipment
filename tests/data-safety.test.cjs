...
test('all visible version writers use the central version',()=>{
  const config=fs.readFileSync(path.join(root,'app-config.js'),'utf8');assert.match(config,/version:'2\.3\.10'/);
  ...
  assert.equal(JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')).version,'2.3.10');
});
...
test('Google Play wrapper preparation stays aligned with the web release',()=>{
  ...
  assert.equal(twa.appVersion,'2.3.10');
  assert.equal(twa.appVersionCode,240);
...
test('native mobile shell and theme are shipped in both HTML and offline cache',()=>{
  ...
  assert.match(html,/app-theme\.css\?v=5/);assert.match(html,/app-config\.js\?v=6/);assert.match(html,/native-ui\.js\?v=4/);assert.match(sw,/app-theme\.css\?v=5/);assert.match(sw,/app-config\.js\?v=6/);assert.match(sw,/native-ui\.js\?v=4/);
