...
test('all visible version writers use the central version',()=>{
  const config=fs.readFileSync(path.join(root,'app-config.js'),'utf8');assert.match(config,/version:'2\.4\.12'/);
  ...
  assert.equal(JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')).version,'2.4.12');
});
...
test('Google Play native shell stays aligned with the web release',()=>{
  ...
  assert.equal(twa.appVersion,'2.4.12');
  assert.equal(twa.appVersionCode,262);
  ...
  assert.match(gradle,/versionCode 262/);
...
test('native mobile shell and theme are shipped in both HTML and offline cache',()=>{
  ...
  assert.match(html,/app-theme\.css\?v=6/);assert.match(html,/app-config\.js\?v=16/);assert.match(html,/native-ui\.js\?v=5/);assert.match(sw,/app-theme\.css\?v=6/);assert.match(sw,/app-config\.js\?v=16/);assert.match(sw,/native-ui\.js\?v=5/);
