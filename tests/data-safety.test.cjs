...
test('all visible version writers use the central version',()=>{
  const config=fs.readFileSync(path.join(root,'app-config.js'),'utf8');assert.match(config,/version:'2\.3\.8'/);
  ...
  assert.equal(JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8')).version,'2.3.8');
});
...
test('Google Play wrapper preparation stays aligned with the web release',()=>{
  ...
  assert.equal(twa.appVersion,'2.3.8');
  assert.equal(twa.appVersionCode,238);
