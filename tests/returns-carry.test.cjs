...
function harness(state,returns={}){
  const data=new Map([['combatEquipmentReturnsV1',JSON.stringify({current:returns})]]);
  const context={state,localStorage:{getItem:key=>data.get(key)??null,setItem:(key,value)=>data.set(key,value)},window:{}};
  vm.createContext(context);
  vm.runInContext(source.slice(0,source.indexOf('  function setAll'))+'\nwindow.test={carryEquipment,missingCurrent};})();',context);
