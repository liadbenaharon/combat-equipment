...
(function(){
  ...
  function transferEquipment(fromName,toName){
    ...
    const info=selectedInfo();let changed=false;
    (info.equipment||[]).forEach(eq=>(eq.assignments||[]).forEach(a=>{if(norm(a.name)===norm(fromName)){a.name=toName;changed=true}}));
    ...
    if(selectedWorkout==='current'){
      ...
      const history=historyItems(),item=history.find(x=>x===info||`history:${x.id}`===selectedWorkout);
      if(item){const people={};(item.equipment||[]).forEach(eq=>(eq.assignments||[]).forEach(a=>{(people[a.name]??=[]).push({name:eq.name,qty:Number(a.qty||0)})}));item.people=Object.entries(people).map(([name,items])=>({name,items}));localStorage.setItem('combatEquipmentHistoryV1',JSON.stringify(history));if(typeof renderHistory==='function')renderHistory()}
    }
    setAsked(fromName,true);renderAttendance();return true;
  }
  function askTransfer(encoded){let from='';try{from=decodeURIComponent(encoded)}catch{from=encoded}const to=prompt(`למי להעביר את כל הציוד של ${from}?`,'');if(to===null)return;if(!clean(to))return alert('יש להזין שם של מקבל הציוד');if(!transferEquipment(from,to))alert('לא הצלחתי להעביר את הציוד')}
  ...
  document.getElementById('noShowWarnings')?.addEventListener('click',e=>{
    ...
    if(button.hasAttribute('data-transfer'))askTransfer(encodeURIComponent(button.dataset.transfer));
  });
  ...
  renderAttendance();decorateTraineeChoices();document.querySelectorAll('.app-version,.app-version-fixed').forEach(el=>el.remove());const appTitle=document.querySelector('.headline h1');if(appTitle){const version=document.createElement('span');version.className='app-version-fixed';version.dir='ltr';version.textContent='v'+(window.COMBAT_APP?.version||'2.3.7');appTitle.append(' ',version)}
})();
