(function(){
  ...
  function addAbout(){
    ...
    const card=document.createElement('section');card.id='aboutApp';card.className='card about-card';card.innerHTML=`<h2>האפליקציה שלי</h2><p class="mini">מותאמת לעבודה מהירה בשטח, בעברית וגם ללא אינטרנט.</p><div class="about-grid"><div class="about-metric"><span>גרסה מותקנת</span><strong dir="ltr">v${APP.version}</strong></div><div class="about-metric"><span>שמירת מידע</span><strong>במכשיר בלבד</strong></div></div><div class="release-note"><div><b>עדכון v${APP.version}</b><div class="mini">עיצוב Android חדש, שמירה בטוחה ועדכונים ברורים</div></div><span aria-hidden="true">✨</span></div><button id="installApp" class="btn primary install-app" hidden>התקנת האפליקציה במכשיר</button><button id="persistStorage" class="btn storage-persist">הגנה מוגברת על הנתונים</button>`;tools?summary.insertBefore(card,tools):summary.appendChild(card);
  ...
  function bindNativeFeedback(){document.addEventListener('click',event=>{if(event.target.closest('.unit,.return-check,[data-step]'))navigator.vibrate?.(12)});window.addEventListener('error',()=>toast('משהו השתבש. הנתונים שלך נשמרו.','error',4200));window.addEventListener('unhandledrejection',()=>toast('לא הצלחנו להשלים את הפעולה. נסו שוב.','error',4200))}
