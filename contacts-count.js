...
(function(){
  ...
  function savedContactCount(){
    try{
      ...
      if(raw!==null){const contacts=JSON.parse(raw);return Array.isArray(contacts)?contacts.length:0}
  ...
  function updateCount(){const summary=document.querySelector('.contacts-card > summary');if(summary)summary.textContent=`אנשי קשר ל־WhatsApp (${savedContactCount()})`}
  function attachObserver(){updateCount();const list=document.getElementById('contactsList');if(list){const observer=new MutationObserver(updateCount);observer.observe(list,{childList:true,subtree:true})}}
